...
const convertNode =
  ...
  async (node: SceneNode): Promise<string> => {
    if ((node as any).canBeFlattened && !nodeHasImageFill(node)) {
      if (settings.embedVectors) {
        ...
        const srcUrl = await getVectorSrc(node, settings);
        const builder = new TailwindDefaultBuilder(node, settings)
          .commonPositionStyles()
          .commonShapeStyles();
        const cleanAlt = (node.name || "icon").replace(/"/g, "&quot;");
        const src = ` src="${srcUrl}" alt="${cleanAlt}"`;
    ...
    switch ((node as any).type) {
      case "RECTANGLE":
      case "ELLIPSE":
        return await tailwindContainer(node, "", "", settings);
      case "GROUP":
...
const tailwindFrame = async (
  ...
  return await tailwindContainer(node, childrenStr, combinedProps, settings);
};
...
export const tailwindContainer = async (
  ...
  if (nodeHasImageFill(node)) {
    const imageURL = await getImageSrc(node, settings);
    const imageFills = getImageFills(node);
    const scaleMode = imageFills[0]?.scaleMode || "FILL";
    ...
    if (!("children" in node) || node.children.length === 0) {
      ...
      src = ` src="${imageURL}" alt="${cleanAlt}"`;
      builder.addAttributes(objectFitClass);
    } else {
      builder.addAttributes(
        `bg-[url(${imageURL})] bg-cover bg-center bg-no-repeat`,
      );
    }
  }

  if (!builder.attributes && !additionalAttr && !src) {
    return children;
  }

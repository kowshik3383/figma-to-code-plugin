...
const processNodePair = async (
  ...
  if (settings.embedVectors) {
    const hasImage = nodeHasImageFill(jsonNode);
    const isPureVector = !hasImage && PURE_VECTOR_TYPES.has(jsonNode.type);
    if (!hasImage && (isPureVector || !parentNode?.canBeFlattened)) {
      const isIcon = isPureVector || isLikelyIcon(jsonNode);
